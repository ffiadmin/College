...
//Bind the buffers to their respective data values
	unsigned int stuff[cubeNS::TOTAL];
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, bufferN);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, cubeNS::TOTAL * sizeof(unsigned int), stuff, GL_STATIC_DRAW);
...
//Define the camera projection matrix
	...
	glm::mat4 camera = glm::lookAt(glm::vec3(10.0f, 10.0f, 10.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
//Render the cube (whew, at last)!
	...
	glDrawElements(GL_TRIANGLES, cubeNS::TOTAL, GL_UNSIGNED_INT, 0);
