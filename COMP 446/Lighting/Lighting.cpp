...
private:
	...
	ID3D10EffectVariable* mfxLightVarPara;
	ID3D10EffectVariable* mfxLightVarPt1;
	ID3D10EffectVariable* mfxLightVarSpot;
	ID3D10EffectVariable* mfxLightVarPt2;
...
: D3DApp(hInstance), mLightType(0), mFX(0), mTech(0), mfxWVPVar(0), mfxWorldVar(0), 
  mfxEyePosVar(0), mfxLightVarPara(0), mfxLightType(0), mVertexLayout(0), 
  mEyePos(0.0f, 0.0f, 0.0f), mRadius(75.0f), mTheta(0.0f), mPhi(PI*0.4f), mfxLightVarPt1(0),
  mfxLightVarSpot(0), mfxLightVarPt2(0)
{
...
void LightingApp::initApp()
{
	...
	// Pointlight--position is changed every frame to animate.
	mLights[3].ambient  = D3DXCOLOR(0.4f, 0.4f, 0.4f, 1.0f);
	mLights[3].diffuse  = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	mLights[3].specular = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	mLights[3].att.x    = 0.0f;
	mLights[3].att.y    = 0.1f;
...
void LightingApp::drawScene()
{
	...
	mfxLightVarPara->SetRawValue(&mLights[0], 0, sizeof(Light));
	mfxLightVarPt1->SetRawValue(&mLights[1], 0, sizeof(Light));
	mfxLightVarSpot->SetRawValue(&mLights[2], 0, sizeof(Light));
	mfxLightVarPt2->SetRawValue(&mLights[3], 0, sizeof(Light));
...
//#endif
	...
	mfxLightVarPara  = mFX->GetVariableByName("gLightPara");
	mfxLightVarPt1  = mFX->GetVariableByName("gLightPt1");
	mfxLightVarSpot  = mFX->GetVariableByName("gLightSpot");
	mfxLightVarPt2  = mFX->GetVariableByName("gLightPt2");
